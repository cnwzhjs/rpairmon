...
HOST_ARCH=$(uname -m)
TARGET_ARCH=armv7hf
BUILD_ROOT=build/linux-$TARGET_ARCH-release
OBJECT_ROOT=$BUILD_ROOT/object
OUTPUT_ROOT=$BUILD_ROOT/output
CROSS_FLAGS="--build=${HOST_ARCH}-linux --host=arm-linux-gnueabihf"
...
if [ -e "${OUTPUT_ROOT}/lib/libpng16.a" ]; then
    ...
    ${PROJECT_ROOT}/3rdparty/libpng-1.6.17/configure --prefix=$PROJECT_ROOT/$OUTPUT_ROOT --enable-arm-neon=yes $CROSS_FLAGS || exit 1
...
# build rpairmon
./onemake/onemake.py concurrent=8 scheme=release target_arch=armv7hf
